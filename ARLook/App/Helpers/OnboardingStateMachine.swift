...
class OnboardingStateMachine: ObservableObject {
  ...
  private let initialStates: [OnboardingState] = [
    .tooFewImages, .firstSegmentNeedsWork, .firstSegmentComplete,
  ...
  private let transitions: [OnboardingState: [Transition]] = [
    ...
    .flippingObjectNotRecommended: [
      (
        inputs: [.continue(isFlippable: true), .continue(isFlippable: false)],
        destination: .captureFromLowerAngle
      ),
    ...
    .secondSegmentComplete: [
      (inputs: [.continue(isFlippable: true)], destination: .flipObjectASecondTime),
      (inputs: [.continue(isFlippable: false)], destination: .captureFromHigherAngle),
    ],

    .flipObjectASecondTime: [
      (inputs: [.finish], destination: .reconstruction),
      (
        inputs: [.continue(isFlippable: true), .continue(isFlippable: false)],
  ...
  func enter(_ input: OnboardingUserInput) -> Bool {
    ...
    if transitions.isEmpty {
      ...
      print("No transitions available for '\(stateString)'")
      return false
    }

    guard
      let destinationState = transitions.first(where: { $0.inputs.contains(where: { $0 == input }) }
      )?.destination
    else {
      let stateString = "\(currentState)"
      let inputString = "\(input)"
      print("No transition from '\(stateString)' for: \(inputString)")
      return false
    }
  ...
  func reset(to state: OnboardingState) -> Bool {
    guard initialStates.contains(state) else {
      ...
      print("Invalid internal state '\(stateString)'.")
      return false
    }
    currentState = state
    return true
  ...
  ) -> [(inputs: [OnboardingUserInput], destination: OnboardingState)] {
    guard let transitions = transitions[state] else {
      let stateString = "\(state)"
      print("No transition exists for '\(stateString)'.")
      return []
    }
    return transitions
  }
